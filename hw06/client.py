...
def client_sender(servers, num_messages):
    ...
    [send_msg_to_server_async(servers[PRIMARY_SERVER_ID], 'chat_msg_client ' + msg_encoded) for msg_encoded in messages]
...
def client_receiver(servers, num_messages, target_msg_index):
    bitmasks = [random.randint(0, 2**num_messages) for _ in range(len(servers)-1)]

    tmp = 2**target_msg_index
    for bitmask in bitmasks:
        tmp = tmp  ^ bitmask

    bitmasks.append(tmp)

    servers_resps = [send_msg_to_server(servers[i], 'pir_req {}'.format(bitmasks[i])) for i in range(len(bitmasks))]
    print(servers_resps)
    result = servers_resps[0]
    for r in servers_resps[1:]:
        result = xor(result, r)
